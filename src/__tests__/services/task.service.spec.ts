...
/* ═══════════════════════════════════════════════════════════════════════════
   Angular core mock — bypasses JIT, provides working inject() via registry.
   ═══════════════════════════════════════════════════════════════════════════ */
vi.mock("@angular/core", () => {
  // Angular signal: sig() returns value; sig.value also returns value.
  ...
  // Angular computed: sig() returns cached value directly (callable signal).
  // sig.value also returns cached value (lazy init on first access).
  // Automatically recomputes when global version changes (signals were modified).
  const mkComputed = (fn: () => unknown) => {
    let cached: unknown;
    let init = false;
    let versionAtInit = -1;
    const sig: any = () => {
      const currentVersion = _hoistedState.getVersion();
      if (!init || versionAtInit !== currentVersion) {
        cached = fn();
        init = true;
        versionAtInit = currentVersion;
      }
      return cached;
    };
    Object.defineProperty(sig, "value", {
  ...
  return {
    signal: mkSignal,
    computed: mkComputed,
    Injectable: () => (target: new (...args: unknown[]) => unknown) => target,
    inject: (token: any) => {
      // Support both class tokens and string tokens
      return _hoistedState.map.get(token) ?? _hoistedState.map.get(token?.name);
    },
    Injector: class {
    ...
    Pipe: () => (target: new (...args: unknown[]) => unknown) => target,
    Directive: () => (target: new (...args: unknown[]) => unknown) => target,
  };
});
