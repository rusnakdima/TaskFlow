...
impl MongoRepoService {
    ...
    pub async fn find_many(
        ...
        let provider = self.provider.as_ref().ok_or_else(|| err_response("MongoDB not available"))?;
        provider.find_many(table, filter).await.map_err(|e| err_response(&e.to_string()))
    }

    pub async fn find_by_id(&self, table: &str, id: &str) -> Result<Option<Value>, ResponseModel> {
        let provider = self.provider.as_ref().ok_or_else(|| err_response("MongoDB not available"))?;
        provider.find_by_id(table, id).await.map_err(|e| err_response(&e.to_string()))
    }

    pub async fn insert(&self, table: &str, data: Value) -> Result<Value, ResponseModel> {
        let provider = self.provider.as_ref().ok_or_else(|| err_response("MongoDB not available"))?;
        provider.insert(table, data).await.map_err(|e| err_response(&e.to_string()))
    }

    pub async fn update(&self, table: &str, id: &str, data: Value) -> Result<Value, ResponseModel> {
        let provider = self.provider.as_ref().ok_or_else(|| err_response("MongoDB not available"))?;
        provider.update(table, id, data).await.map_err(|e| err_response(&e.to_string()))
    }

    pub async fn delete(&self, table: &str, id: &str) -> Result<(), ResponseModel> {
        let provider = self.provider.as_ref().ok_or_else(|| err_response("MongoDB not available"))?;
        provider.delete(table, id).await.map_err(|e| err_response(&e.to_string()))
    }
        ...
        for doc in &mut docs {
            if let Some(obj) = doc.as_object_mut() {
                if !obj.contains_key("_collection") {
                    obj.insert("_collection".to_string(), Value::String(collection.to_string()));
                }
            }
        }
        docs
