...
impl QrAuthService {
    ...
    pub async fn generateQrToken(&self, username: Option<&str>) -> Result<ResponseModel, ResponseModel> {
        ...
        if let Some(ref mongoProvider) = self.mongodbProvider {
            match mongoProvider.create("qr_tokens", qr_token_json.clone()).await {
                Ok(_) => {
        ...
        match self.jsonProvider.create("qr_tokens", qr_token_json.clone()).await {
            Ok(result) => {
                eprintln!("[QR] Successfully stored token in local DB, result: {}", result);
            }
            Err(e) => {
    ...
    pub async fn approveQrToken(&self, token: &str, approving_username: &str) -> Result<ResponseModel, ResponseModel> {
        eprintln!("[QR] approveQrToken called with token: '{}', username: '{}'", token, approving_username);
        ...
        eprintln!("[QR] Token approved by {} for user {}", approving_username, approving_username);
    ...
    pub async fn toggleQrLogin(&self, username: &str, enabled: bool) -> Result<ResponseModel, ResponseModel> {
        eprintln!("[QR] toggleQrLogin called for {}: {}", username, enabled);
        Ok(successResponse(if enabled { "QR login enabled" } else { "QR login disabled" }))
    }
    ...
    async fn findQrToken(&self, token: &str) -> Result<QrToken, ResponseModel> {
        ...
        if let Some(ref mongoProvider) = self.mongodbProvider {
            match mongoProvider.getAll("qr_tokens", Some(filter.clone())).await {
                Ok(results) => {
    ...
    async fn saveQrToken(&self, token: &QrToken) -> Result<(), ResponseModel> {
        ...
        // QR login is cross-device, so MongoDB is primary store
        if let Some(ref mongoProvider) = self.mongodbProvider {
            if let Err(e) = mongoProvider.update("qr_tokens", &token.id, token_val.clone()).await {
                eprintln!("[QR] Failed to update token in MongoDB: {}", e);
            }
        }

        // Also update local cache
        if let Err(e) = self.jsonProvider.update("qr_tokens", &token.id, token_val).await {
    ...
    pub async fn completeQrLogin(&self, token: &str) -> Result<ResponseModel, ResponseModel> {
        ...
        let username = qr_token.username.ok_or_else(|| {
            errResponse("QR token has no username")
        })?;
        ...
        let user_val = match self.jsonProvider.getAll("users", Some(filter.clone())).await {
        ...
        let user_val = if let Some(val) = user_val {
            ...
            match mongoProvider.getAll("users", Some(filter)).await {
                Ok(users) => users.first().cloned().ok_or_else(|| {
                    errResponse(&format!("User '{}' not found in database", username))
                })?,
        ...
        // Generate JWT token
        let token = self.tokenService.generateToken(&user.id, &user.username, &user.role)?;
