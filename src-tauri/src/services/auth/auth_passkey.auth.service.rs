...
impl AuthPasskeyService {
  ...
  pub async fn init_registration(&self, username: &str) -> Result<ResponseModel, ResponseModel> {
    ...
    let mut challenge_store = self.challenge.lock().unwrap_or_else(|e| e.into_inner());
  ...
  pub async fn check_profile_exists(
    ...
    check_profile_exists(&self.json_provider, self.mongodb_provider.clone(), user_id).await
  }
}
