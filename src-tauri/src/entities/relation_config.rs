...
impl RelationConfig {
  pub fn todos_relations() -> Vec<(&'static str, RelationDef)> {
    vec![
      ("tasks", RelationDef::one_to_many("tasks", "tasks", "todoId")),
      ("user", RelationDef::many_to_one("user", "users", "userId")),
      ("categories", RelationDef::many_to_many("categories", "categories", "categories")),
      ("assignees", RelationDef::many_to_many("assignees", "profiles", "assignees")),
      ("assigneesProfiles",
  ...
  pub fn tasks_relations() -> Vec<(&'static str, RelationDef)> {
    vec![
      ("subtasks", RelationDef::one_to_many("subtasks", "subtasks", "taskId")),
      ("comments", RelationDef::one_to_many("comments", "comments", "taskId")),
      ("assignees", RelationDef::many_to_many("assignees", "profiles", "assignees")),
  ...
  pub fn subtasks_relations() -> Vec<(&'static str, RelationDef)> {
    vec![
      ...
      ("comments", RelationDef::one_to_many("comments", "comments", "subtaskId")),
      ("assignees", RelationDef::many_to_many("assignees", "profiles", "assignees")),
    ]
  }
  ...
  pub fn comments_relations() -> Vec<(&'static str, RelationDef)> {
    vec![
      ("task", RelationDef::many_to_one("task", "tasks", "taskId")),
      ("subtask", RelationDef::many_to_one("subtask", "subtasks", "subtaskId")),
    ]
  }
  ...
  pub fn users_relations() -> Vec<(&'static str, RelationDef)> {
    vec![
      ("profile", RelationDef::many_to_one("profile", "profiles", "profileId")),
    ]
  }
  ...
  pub fn needs_user_projection(table: &str, path: &str) -> bool {
    matches!((table, path),
      ("todos", "user") | ("todos", "assigneesProfiles") |
      ("profiles", "user") |
      ("tasks", "assignees") |
      ("subtasks", "assignees") |
      ("comments", "author")
  ...
  pub fn get_nested_relation(table: &str, base: &str, nested: &str) -> Option<RelationDef> {
    match (table, base, nested) {
      ("todos", "tasks", "subtasks") => Some(RelationDef::one_to_many("subtasks", "subtasks", "taskId")),
      ("todos", "tasks", "comments") => Some(RelationDef::one_to_many("comments", "comments", "taskId")),
      ("tasks", "subtasks", "comments") => Some(RelationDef::one_to_many("comments", "comments", "subtaskId")),
      ("subtasks", "subtasks", "comments") => Some(RelationDef::one_to_many("comments", "comments", "subtaskId")),
      _ => None,
    }
  }
...
mod tests {
  ...
  fn test_excluded_fields_for_user() {
    let excluded = <crate::entities::user_entity::UserEntity as FrontendProjection>::excluded_fields();
